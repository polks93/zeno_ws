...
class RecordData:
    def __init__(self):
        ...
        self.best_samples_timestamps = []
        self.best_samples_x_values = []
        self.best_samples_y_values = []
        self.best_samples_theta_values = []
        ...
        rospy.Subscriber('/best_samples', MarkerArray, self.samples_callback)
    ...
    def samples_callback(self, msg):
        if self.recording:
            elapsed_time = time.time() - self.start_time
            if msg.markers[0].pose.position.x == 0.0 and msg.markers[0].pose.position.y == 0.0:
                return
            self.best_samples_timestamps.append(elapsed_time)
            self.best_samples_x_values.append(msg.markers[0].pose.position.x)
            self.best_samples_y_values.append(msg.markers[0].pose.position.y)
            q = msg.markers[0].pose.orientation
            _, _, theta = tf.transformations.euler_from_quaternion([q.x, q.y, q.z, q.w])
            self.best_samples_theta_values.append(theta)
    ...
    def save_data(self, msg):
        ...
        best_samples_timestamps = np.array(self.best_samples_timestamps)
        best_samples_x = np.array(self.best_samples_x_values)
        best_samples_y = np.array(self.best_samples_y_values)
        best_samples_theta = np.array(self.best_samples_theta_values)
